...
app.post('/api/payments/create-preference', async (req, res) => {
  try {
    ...
    const baseUrl = process.env.RENDER_EXTERNAL_URL || process.env.FRONTEND_URL || 'https://bicicamilo.onrender.com';
  ...
  try {
    ...
    try {
      ...
      await resend.emails.send({
        from: 'Bicicamilo Store <onboarding@resend.dev>',
        to: 'pily241@hotmail.com',
...
async function sendAdminPaymentNotificationEmail(order) {
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: process.env.ADMIN_EMAIL || 'ventas@bicicamilo.com',
